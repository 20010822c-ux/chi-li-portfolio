...
export default function ProjectModal({ children, modal }: ProjectModalProps) {
  ...
  const modalOverlay = isOpen ? (
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/80 px-4 py-6 backdrop-blur-sm"
      onClick={() => setIsOpen(false)}
    >
      <div
        className="relative z-10 max-h-[calc(100vh-3rem)] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-cyan-200/20 bg-slate-950/95 p-5 shadow-[0_30px_120px_rgba(0,0,0,0.65)] sm:p-7"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {modal.documentUrl ? (
          <div className="relative z-10 mt-8 flex flex-col gap-3 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
  ...
  return (
    <>
      ...
      {modalOverlay && typeof document !== "undefined" ? createPortal(modalOverlay, document.body) : null}
    </>
  );
}
